...
func (p *ConvertCliToOpenAIParams) ensureBaseChunkTemplate(defaultModel string, createdAt int64, responseID string) {
	...
	if createdAt != 0 {
		template, _ = sjson.Set(template, "created", createdAt)
	}
	if responseID != "" {
		template, _ = sjson.Set(template, "id", responseID)
	}
...
func ConvertCodexResponseToOpenAI(_ context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	if *param == nil {
		...
		p.ensureBaseChunkTemplate(modelName, 0, "")
		*param = p
	}
	...
	if dataType == "response.created" {
		...
		params.ensureBaseChunkTemplate(modelName, params.CreatedAt, params.ResponseID)
		return [][]byte{}
	}
	...
	if usageResult := rootResult.Get("response.usage"); usageResult.Exists() {
		...
		if cacheWriteTokensResult := usageResult.Get("input_tokens_details.cache_write_tokens"); cacheWriteTokensResult.Exists() {
			template, _ = sjson.Set(template, "usage.prompt_tokens_details.cached_creation_tokens", cacheWriteTokensResult.Int())
		}
	...
	if dataType == "response.reasoning_summary_text.delta" {
		...
		} else if params.FunctionCallIndex != -1 {
